...
class Auth_Controller_students extends RestApi_Controller 
{
    ...
    function login() 
    {
        ...
        if($this->form_validation->run())
        {
            ...
            //  $data = array('email'=>$email,'password'=> sha1($password));
             $login_credential = $this->Api_Students_Model->login_credential($email, $password);
             if($login_credential != false) 
             {
                  ...
                  $getUser = $this->Api_Students_Model->getUserNameByRoleID($login_credential->role, $login_credential->user_id);
