...
<section class="panel">
    <div class="tabs-custom">
        <ul class="nav nav-tabs">
            ...
            <li>
                <a href="#history" data-toggle="tab"><i class="fas fa-dollar-sign"></i>
                    <?=translate('payment_history')?></a>
            </li>
            <?php endif; ?>
            <?php if (get_permission('collect_fees', 'is_add') && $invoice['status'] != 'total'): ?>
            <li>
                <a href="#collect_fees" data-toggle="tab"><i class="fas fa-hand-holding-usd"></i>
                    <?=translate('collect_fees')?></a>
            </li>
            <?php endif; ?>
            <?php if (get_permission('collect_fees', 'is_add') && $invoice['status'] != 'total'): ?>
            <li>
        ...
        <div class="tab-content">
            <div id="invoice" class="tab-pane <?=empty($this->session->flashdata('pay_tab')) ? 'active' : ''; ?>">
                <div id="invoice_print">
                    <div class="invoice">
                            ...
                            <div class="row">
                                <div class="col-xs-6">
                                    <div class="ib">
                                        ...
                                        <img height="85"
                                            src="<?=$this->application_model->getBranchImage($basic['branch_id'], 'printing-logo')?>"
                                            alt="RamomCoder Img" />
                                        <?php }else{ ?>
                                        <img src="<?=$this->application_model->getBranchImage($basic['branch_id'], 'printing-logo')?>"
                                            alt="RamomCoder Img" />
                                        <?php } ?>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-xs-6">
                                    <div class="bill-data">
                                        ...
                                        <address>
                                            <?php
                                            if (get_loggedin_branch_id() == 4) { 
                                                echo $basic['first_name'] . ' ' . $basic['last_name'] . ' ' . $basic['fathername'] . '<br>';
                                            }else{ 
                                                echo $basic['last_name'] . ' ' . $basic['first_name'] . ' ' . '<br>';
                                            } 
											// echo $basic['last_name'] . ' ' . $basic['first_name'] . ' ' . $basic['fathername'] . '<br>';
											...
											echo translate('parent_mobile_no.') . ' : ' . $basic['parent_mobileno'] .'<br>'; 
                                            echo translate('gr_number').' : ' .$basic['register_no'] .'<br>';
                                            echo translate('roll_number').' : ' .$basic['roll_no'];
											?>
                                        </address>
                                    </div>
                                </div>
                                <div class="col-xs-6">
                                    <div class="bill-data text-right">
                                        ...
                                        <address>
                                            <?php 
                                            if (get_loggedin_branch_id() == 4) {
                                                if ($basic['class_name'] >= '1' && $basic['class_name'] <= '8') {
                                                    echo $basic['school_name'] . "<br/>";
                                                }
                                                if ($basic['class_name'] === '9' || $basic['class_name'] === '10') {
                                                    echo 'ULTRA VISION SCHOOL(Secondary) <br/>';
                                                }
                                                if ($basic['class_name'] === '11' || $basic['class_name'] === '12') {
                                                    echo 'ULTRA VISION SCHOOL(Higher Secondary) <br/>';
                                                }
                                            }else{
                                                echo $basic['school_name'] . "<br/>";
                                            }
                        ...
                        <button type="button" class="btn btn-default btn-sm mb-sm hidden-print" id="collectFees"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="group" colspan="10">
                                            <strong><?php echo get_type_name_by_id('fee_groups', $row['group_id']) ?></strong><img
                                                class="group" src="<?php echo base_url('assets/images/arrow.png') ?>">
                                            <?=$row['school_year']?>
                                        </td>
                                    </tr>
                                    <?php } ?>
                                    <tr>
                                        <td class="hidden-print checked-area">
                                            <div class="checkbox-replace">
                                                <label class="i-checks"><input type="checkbox" name="cb_invoice"
                                                        value="<?php echo $row['amount']; ?>"
                                                        data-allocation-id="<?php echo $row['allocation_id'] ?>"
                                                        data-fee-type-id="<?php echo $row['fee_type_id'] ?>"
                                                        checked><i></i></label>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <?php echo $currency_symbol . number_format($balance, 2, '.', '');?></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                            ...
                            <div class="row">
                                <div class="col-md-5 col-xs-12 pull-right">
                                    <ul class="amounts">
                                        ...
                                        <li><strong><?=translate('total_paid')?> (with fine) :</strong>
                                            <?=$currency_symbol . number_format($total_paid + $total_fine, 2, '.', ''); ?>
                                        </li>
                                        <li>
                                        ...
                                        <?php else: 
											...
											?>
                                        <li>
                                            <strong><?=translate('total_paid')?> (<?=translate('with_fine')?>) :
                                            </strong>
                                            <?php
											$numberSPELL = "";
											if ($extINTL == true) {
                    ...
                    <div class="text-right mr-lg hidden-print">
                        <button id="invoicePrint" class="btn btn-default ml-sm"
                            data-loading-text="<i class='fas fa-spinner fa-spin'></i> Processing"><i
                                class="fas fa-print"></i> <?=translate('print')?></button>
                    </div>
                </div>
            </div>
            <?php if ($invoice['status'] != 'unpaid'): ?>
            <div class="tab-pane" id="history">
                <div id="payment_print">
                    <div class="invoice payment">
                        <header class="clearfix">
                            <div class="row">
                                ...
                                <div class="col-md-6 text-right">
                                    <h4 class="mt-none mb-none text-dark">Invoice No
                                        #<?php echo $invoice['invoice_no']?></h4>
                                    <p class="mb-none">
                                        <span class="text-dark"><?=translate('date')?> : </span>
                        ...
                        <button type="button" class="btn btn-default btn-sm mb-sm hidden-print" id="selected_revert"
                            data-loading-text="<i class='fas fa-spinner fa-spin'></i> Processing">
                            ...
                            <table class="table invoice-items" id="paymentHistory">
                                <thead>
                                    <tr class="h5 text-dark">
                                        ...
                                        <th id="cell-item" class="text-weight-semibold hidden-print">
                                            <?=translate('collect_by')?></th>
                                        ...
                                        <!--<th id="cell-qty" class="text-weight-semibold"><?=translate('Payment_details')?></th>-->
                                        <th id="cell-price" class="text-weight-semibold"><?=translate('amount')?></th>
                                        <th id="cell-price" class="text-weight-semibold"><?=translate('discount')?></th>
                                        <th id="cell-price" class="text-weight-semibold"><?=translate('fine')?></th>
                                        <th id="cell-price" class="text-weight-semibold"><?=translate('paid')?></th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="hidden-print checked-area">
                                            <div class="checkbox-replace">
                                                <label class="i-checks"><input type="checkbox" name="cb_feePay"
                                                        value="<?php echo $row['id']; ?>" checked><i></i></label>
                                            </div>
                                        </td>
                                        ...
                                        <?php 
                                            ...
                                            //     echo '-';
                                            // } 
                                            // elseif($row['payvia'] == 'Cheque'){ 
                                            //     // echo $row['pay_via_details'];
                                            //     $jsonString = $row['pay_via_details'];
                                            //     $data = json_decode($jsonString, true);

                                            //     echo '<div style="border-bottom:1px solid black;">Cheque No.: '.$data['chequeNo'] . '</div>';     // Output: 1
                                            //     echo '<div style="border-bottom:1px solid black;">Bank Name: ' . $data['bankName'] . '</div>';     // Output: 123
                                            //     echo 'date : ' .$data['chequeDate']; 
                                            // }
                                            // elseif($row['payvia'] == 'Card'){
                                            //     $jsonString = $row['pay_via_details'];
                                            //     $data = json_decode($jsonString, true);
                                            //     echo '<div >Transaction No. : '.$data['transactionNo'] . '</div>';
                                            // }elseif($row['payvia'] == 'Other'){
                                            //     echo '-';
                                            // }else{
                                            //     $jsonString = $row['pay_via_details'];
                                            //     $data = json_decode($jsonString, true);
                                            //     echo '<div >Transaction Id. : '.$data['transferId'] . '</div>';
                                            // }
                                        ?>
                                        <!--</td>-->
                        ...
                        <div class="invoice-summary text-right mt-lg hidden-print">
                            <div class="row">
                                <div class="col-md-5 col-xs-12 pull-right">
                                    <ul class="amounts">
                                        <li><strong><?=translate('sub_total')?> :</strong>
                                            <?=$currency_symbol . number_format($total_paid + $total_discount, 2, '.', ''); ?>
                                        </li>
                                        <li><strong><?=translate('discount')?> :</strong>
                                            <?=$currency_symbol . number_format($total_discount, 2, '.', ''); ?></li>
                                        <li><strong><?=translate('paid')?> :</strong>
                    ...
                    <div class="text-right mr-lg hidden-print">
                        <button id="paymentPrint" class="btn btn-default"
                            data-loading-text="<i class='fas fa-spinner fa-spin'></i> Processing"><i
                                class="fas fa-print"></i> <?=translate('print')?></button>
                    </div>
                </div>
            ...
            <div id="collect_fees" class="tab-pane">
                ...
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('fees_type')?> <span
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <input type="text" class="form-control" data-plugin-datepicker
                            data-plugin-options='{"todayHighlight" : true, "endDate": "today"}' name="date"
                            value="<?=date('Y-m-d')?>" autocomplete="off" />
                        <span class="error"></span>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('amount')?> <span
                            class="required">*</span></label>
                    <div class="col-md-6">
                        <input type="text" class="form-control" name="amount" id="feeAmount" value=""
                            autocomplete="off" />
                        <span class="error"></span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <input type="text" class="form-control" name="fine_amount" id="fineAmount" value="0"
                            autocomplete="off" />
                        <span class="error"></span>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('payment_method')?> <span
                            class="required">*</span></label>
                    <div class="col-md-6">
                        <?php
	    							$payvia_list = $this->app_lib->getSelectList('payment_types');
	    							echo form_dropdown("pay_via", $payvia_list, set_value('pay_via'), "class='form-control' id='typeID' data-plugin-selectTwo data-width='100%'
                ...
                <div class="form-group">
                    <div class="row hidden-div" id="cheque">
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('cheque_number')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="chequeNo" id="chequeNo" value=""
                                    autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('bank_name')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('cheque_date')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                    ...
                    <div class="row hidden-div" id="card">
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('transaction_number')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="transactionNo" id="transactionNo" value=""
                                    autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
                    </div>
                    <div class="row hidden-div" id="banktransfer">
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('transaction_id')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="transferId" id="transferId" value=""
                                    autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
                ...
                <?php
                        ...
                        ?>
                <div class="form-group">
                    <label class="col-md-3 control-label"><?php echo translate('account'); ?> <span
                            class="required">*</span></label>
                    <div class="col-md-6">
                        <?php
            ...
            <div id="fully_paid" class="tab-pane">
                ...
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('amount')?> <span
                            class="required">*</span></label>
                    <div class="col-md-6">
                        <input type="text" class="form-control" name="amount" id="feeAmount"
                            value="<?=number_format($total_balance, 2, '.', '')?>" autocomplete="off" disabled />
                        <span class="error"></span>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('fine')?></label>
                    <div class="col-md-6">
                        <input type="text" class="form-control" name="fine_amount" id="fineAmount"
                            value="<?=number_format($fully_total_fine, 2, '.', '')?>" autocomplete="off" disabled />
                        <span class="error"></span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <?php
	    							$payvia_list = $this->app_lib->getSelectList('payment_types');
	    							echo form_dropdown("pay_via", $payvia_list, set_value('pay_via'), "class='form-control' id='typeIDfull' data-plugin-selectTwo data-width='100%'
	    							data-minimum-results-for-search='Infinity' ");
	    						?>
                        <span class="error"></span>
                    </div>
                ...
                <div class="form-group">
                    <div class="row hidden-div" id="chequefull">
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('cheque_number')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="chequeNofull" id="chequeNo" value=""
                                    autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('bank_name')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="bankNamefull" id="bankName" value=""
                                    autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('cheque_date')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="chequeDatefull" id="chequeDate"
                                    data-plugin-datepicker
                                    data-plugin-options='{"todayHighlight" : true, "endDate": "today"}'
                                    value="<?=date('Y-m-d')?>" autocomplete="off" />
                                <span class="error"></span>
                    ...
                    <div class="row hidden-div" id="cardfull">
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('transaction_number')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="transactionNofull" id="transactionNo"
                                    value="" autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
                    </div>
                    <div class="row hidden-div" id="banktransferfull">
                        <div class="form-group">
                            <label class="col-md-3 control-label"><?=translate('transaction_id')?> <span
                                    class="required">*</span></label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" name="transferIdfull" id="transferId" value=""
                                    autocomplete="off" />
                                <span class="error"></span>
                            </div>
                        </div>
        ...
        <footer class="panel-footer">
            <div class="row">
                <div class="col-md-12 text-right">
                    <button type="submit" class="btn btn-default"
                        data-loading-text="<i class='fas fa-spinner fa-spin'></i> Processing">Fee Payment</button>
                </div>
            </div>
        </footer>
        <?php echo form_close();?>
...
$('#typeID').on('change', function() {
    ...
    if (val == 4 || val == 6 || val == 7 || val == 8 || val == 9 || val == 10 || val == 11 || val == 12 ||
...
$('#typeIDfull').on('change', function() {
    var val = $(this).val();
    if (val == 1 || val == 5) {
        $("#chequefull").hide('slow');
        $("#cardfull").hide('slow');
        $("#banktransferfull").hide('slow');
        $("#cashfull").show('slow');
    }
    if (val == 2) {
        $("#chequefull").hide('slow');
...
$('#invoicePrint').on('click', function(e) {
    ...
    if (arrayData.length === 0) {
        ...
        $.ajax({
            ...
            complete: function() {
                fn_printElem2copy('invoice_print');
                $btn.button('reset');
            }
        });
    }
...
$('#paymentPrint').on('click', function(e) {
    ...
    if (arrayData.length === 0) {
        ...
        $.ajax({
            ...
            complete: function() {
                fn_printElem2copy('payment_print');
                $btn.button('reset');
            }
        });
    }
...
function fn_printElem2copy(elem, html = false) {
    if (html == false) {
    ...
    frameDoc.document.write(
        '<style> @media print { @page { size: auto; margin-top: 5mm; margin-bottom: 0mm; } body { margin: 0; } }</style>'
    ); // Add this style to remove footer with URL and page number
    frameDoc.document.write('</head><body>');

    // Generate two copies side by side
    frameDoc.document.write('<div style="display: flex; justify-content: space-between">');
    frameDoc.document.write('<div>' + oContent + '</div>');
    frameDoc.document.write('<div>' + oContent + '</div>');
